...
import { contactMessageNotificationEmail } from "@/lib/email/templates";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await prisma.contactMessage.create({
      data: { name: body.name, email: body.email, reason: body.reason, message: body.message },
    });

    const notification = contactMessageNotificationEmail({ ...body, adminUrl: appUrl("/admin/inquiries") });
