...
export async function POST(request: NextRequest) {
  try {
    ...
    // Best-effort — sendEmail never throws, so a missing RESEND_API_KEY or a
    // provider error never fails the request itself; the request is already
    // saved and visible in /admin/inquiries either way.
    await Promise.all([
